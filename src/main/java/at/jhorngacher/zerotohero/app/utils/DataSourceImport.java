...
public class DataSourceImport {
    ...
    public void init() {
        ...
        /**
         *
         * Wir prüfen ob bereits Daten vorhanden sind, falls ja kein Insert!
         *
         */
        Long count = em.createQuery("select count(*) from CarbonData c", Long.class)
                .getSingleResult();

        if (count == 0) {

            try (
                 Reader reader = new InputStreamReader(getClass().getResourceAsStream("/data/owid-co2-data.csv"));
                ...
                while ((line = csvReader.readNext()) != null) {
                    ...
                    // replacement von "." da dies falsch interpretiert wird!
                    ...
                    carbonData.setEmissionYear(Integer.parseInt(line[1]));
            ...
            /**
             * Import eines Default Nutzeracconts Dev-only
             */
            try{
                UserAccount devAccount = new UserAccount();
                devAccount.setAccountEMail("mm@zerotohero.com");
                devAccount.setAccountName("Max Mustermann");
                devAccount.setAccountPassword("Testpasswort123!");

                tx.begin();
                em.persist(devAccount);
                tx.commit();
            } catch (Exception e) {
                tx.rollback();
                e.printStackTrace();
            ...
            System.out.println("Es werden keine weiteren Datensätze mehr importiert! Row count > 0");
        }
    }
}
